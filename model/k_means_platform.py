...
def k_means(sheet_name:list, data:list):
    ...
    labels = []
    for i in range(20):
        km = TimeSeriesKMeans(n_clusters=n_clusters, n_init=10, max_iter=300, metric="dtw",random_state=i)
    ...
    for i in range(max_len1):
        ...
        for x in label1:
            sum += data[x][i]
        represent1.append(int(sum / 3))
    for i in range(max_len2):
        sum = 0
        for x in label2:
            sum += data[x][i]
        represent2.append(int(sum / 3))
    for i in range(max_len3):
        sum = 0
        for x in label3:
            sum += data[x][i]
        represent3.append(int(sum / 3))
    for i in range(max_len4):
        sum = 0
        for x in label4:
            sum += data[x][i]
        represent4.append(int(sum / 3))
